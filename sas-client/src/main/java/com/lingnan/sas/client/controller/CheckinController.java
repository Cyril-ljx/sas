...
import com.lingnan.sas.core.entity.CheckinVO;
...
public class CheckinController {
    ...
    public R validCanCheckIn(@RequestHeader("token") String token){
        int userId=jwtUtil.getUserId(token);//从请求头反向解析获得userID
    ...
    public R checkin(@Valid CheckinVO form, @RequestParam("photo") MultipartFile file, @RequestHeader("token") String token){
        ...
        if(!fileName.endsWith(".jpg")){
            ...
            try{
                ...
                throw new ClientException("图片保存错误");
            }
            finally {
                FileUtil.del(path);
            }
...
/*
    ...
    public R searchMonthCheckin(@Valid @RequestBody SearchMonthCheckinForm form, @RequestHeader("token") String token){
        ...
        if(startDate.isBefore(DateUtil.beginOfMonth(hiredate))){
            throw new ClientException("只能查询考勤之后日期的数据");
        }
        if(startDate.isBefore(hiredate)){
            startDate=hiredate;
        }
